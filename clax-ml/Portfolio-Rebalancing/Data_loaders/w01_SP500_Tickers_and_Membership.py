...
# Only initialize API if credentials are available
api = None
...
def verify_tickers_with_alpaca(tickers: List[str]) -> List[str]:
    ...
    If no API available, return all tickers.
    """
    if api is None:
        logger.warning("No Alpaca API credentials available. Skipping verification and using all Wikipedia tickers.")
        return tickers
    ...
    for symbol in tqdm(tickers, desc="Verifying Tickers"):
        try:
            ...
            if asset.tradable:
                ...
                logger.debug("Ticker %s not tradable", symbol)
