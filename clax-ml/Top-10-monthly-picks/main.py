...
async def get_asset_overview(ticker: str):
    normalized_ticker = _normalize_ticker(ticker)
...
async def get_top10_market_data():
    """
    ...
    for ticker in TOP_10_SP500_TICKERS:
        ...
        # Overview
        try:
            overview_resp = await get_asset_overview(ticker)
        ...
        # Quote
        try:
            quote_resp = await get_asset_quote(ticker)
            ticker_result["quote"] = {
        ...
        # Chart
        try:
            chart_resp = await get_asset_chart(ticker, range="24h", interval="1h")
            ticker_result["chart"] = {
...
async def predict_top10(date: Optional[str] = Query(None, description="Date for prediction in YYYY-MM-DD format. If not specified, automatically uses the latest available date in the data. If a future date is provided, falls back to the latest available date.")):
    """
...
# ----------------- Job Control APIs ----------------- #
@app.post("/jobs/refresh_data", status_code=202, tags=["Jobs"])
...
@app.post("/jobs/retrain_model", status_code=202, tags=["Jobs"])
async def retrain_model(params: RetrainInput, background_tasks: BackgroundTasks):
